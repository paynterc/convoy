...
public class RadarSystem : MonoBehaviour
{
	...
	public ParticleSystem RenderMode;
	public GameObject CenterPoint;
	public float RadarScale=50.0f;
	...
	private ParticleSystem _pe;
	private ParticleSystemRenderer _pr;
	public ParticleSystem.Particle[] _rblips = new ParticleSystem.Particle[0];
	...
	public void Awake ()
	{
		...
		_pe = gameObject.AddComponent<ParticleSystem>() as ParticleSystem;
		_pe.Stop();
	...
	void Update ()
	{
		...
		if (CenterPoint != null) {
			...
			for (int i = 0; i < max; i++) {
                if (!_RadarBlips[i].Target) continue;
                ...
                // Clamp position
                _rblips[i].position = Vector3.ClampMagnitude(_rblips[i].position, maxRange);


                if (!FadeOverDistance) {
					// Specifies Blip Color - With no Opacity, set in inspector
					_rblips [i].startColor = new Color (_RadarBlips [i].BlipColor.r, _RadarBlips [i].BlipColor.g, _RadarBlips [i].BlipColor.b); 
				} else {
					// Specifies Blip Color - With Opacity
					_rblips [i].startColor = new Color (_RadarBlips [i].BlipColor.r, _RadarBlips [i].BlipColor.g, _RadarBlips [i].BlipColor.b, _RadarBlips [i].BlipOpacity);
				}
				
				_rblips [i].startLifetime = 3.0f;
				_rblips [i].remainingLifetime = 3.0f;
				_rblips [i].velocity = Vector3.zero;
				_rblips [i].startSize = _RadarBlips [i].BlipSize;
				...
				if (drawLineHeights) {
                    ...
                    length = Mathf.Clamp(length, maxRange*-1, maxRange);
                    ...
                    line.startColor = _rblips[i].startColor;
                    line.endColor = _rblips[i].startColor;
